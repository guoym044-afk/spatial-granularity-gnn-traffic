...
Table~\ref{tab:smoothing_full} provides the complete smoothing control results with per-seed breakdown.

\begin{table}[t]
\small
\centering
\caption{Smoothing control at $N = 207$: full results}
\label{tab:smoothing_full}
\begin{tabular}{llcccc}
\toprule
& Model & $\sigma{=}0$ & $\sigma{=}1$ & $\sigma{=}2$ & $\sigma{=}3$ \\
\midrule
\multirow{3}{*}{\rotatebox{90}{\tiny Models}}
& DCRNN        & 3.68$\pm$.12 & 1.49$\pm$.02 & 0.58$\pm$.02 & 0.39$\pm$.02 \\
& GraphWaveNet & 3.61$\pm$.08 & 2.19$\pm$.16 & 1.35$\pm$.03 & 1.01$\pm$.04 \\
& LSTGCNN      & 5.08$\pm$.02 & 3.59$\pm$.01 & 2.97$\pm$.00 & 2.89$\pm$.04 \\
\midrule
\multirow{3}{*}{\rotatebox{90}{\tiny Seeds}}
& DCRNN (s42)  & 3.60 & 1.48 & 0.57 & 0.38 \\
& DCRNN (s123) & 3.82 & 1.51 & 0.60 & 0.41 \\
& DCRNN (s456) & 3.63 & 1.47 & 0.58 & 0.39 \\
\midrule
...
Key observations: (1) $\sigma = 1$ drops DCRNN MAE below its $N = 5$ value ($\sim$2.0). (2) $\sigma = 2$ gives DCRNN skill $= +0.57$. (3) Per-seed variance is small ($\pm 0.02$--$0.12$).
...
Across 834{,}560 attention weight samples (all heads, layers, test-set nodes), the mean normalized entropy is $0.993 \pm 0.059$ (median: 1.000, 5th/95th percentiles: 0.871/1.000), confirming that learned attention is statistically indistinguishable from uniform weighting.

\paragraph{RandomGAT baseline.}
Table~\ref{tab:randomgat_app} compares GAT variants on PeMS-Bay.
...
\midrule
RandomGAT    & 1.355$\pm$.002 & $+0.211$ & fixed uniform, 3 seeds \\
Learned GAT  & --- & --- & did not converge \\
\midrule
LSTGCNN      & 1.26$\pm$.06 & $+0.267$ & 10 seeds \\
DCRNN        & 1.27$\pm$.01 & $+0.260$ & 10 seeds \\
STGCN        & 1.23$\pm$.03 & $+0.283$ & 10 seeds \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Convergence failure.}
Learned GAT plateaus within the first few epochs across all configurations (3 seeds, multiple learning rates). Gradient norms in attention parameters decay to near-zero within 10 epochs, while LSTM temporal-layer gradients remain non-zero. The optimizer effectively abandons the attention parameters, consistent with entropy remaining near initialization (uniform).
