...
# ============================================================
# Non-GNN Baselines: TCN and DLinear
# ============================================================

class TCNPredictor(nn.Module):
    """
    Temporal Convolutional Network for time-series forecasting.
    No spatial message passing — each node processed independently.
    """
    def __init__(self, num_nodes, input_dim=1, hidden_dim=64, output_dim=3,
                 num_levels=4, kernel_size=3, dropout=0.1):
        super().__init__()
        self.num_nodes = num_nodes
        self.output_dim = output_dim
    ...
    def forward(self, x, adj=None):
        """
        x: (batch, lookback, num_nodes) — adj ignored
        Returns: (batch, horizon, num_nodes)
        """
        B, T, N = x.shape
        # (B*N, 1, T) for Conv1d
        x_flat = x.permute(0, 2, 1).reshape(B * N, 1, T)
        h = self.tcn(x_flat)
        # Causal: take last T steps
        h = h[:, :, :T]
        h = h[:, :, -1]  # (B*N, hidden)
        out = self.output_proj(h)  # (B*N, horizon)
        return out.reshape(B, N, -1).permute(0, 2, 1)


class DLinearPredictor(nn.Module):
    """
    DLinear: Decomposition + Linear (Zeng et al., AAAI 2023).
    Simple trend-seasonal decomposition followed by linear projection.
    No spatial message passing — each node processed independently.
    """
    def __init__(self, num_nodes, lookback=12, output_dim=3):
        super().__init__()
        self.num_nodes = num_nodes
        self.lookback = lookback
        self.output_dim = output_dim

        # Trend and seasonal linear layers (per-node would be too many params;
        # use shared weights across nodes, same as other baselines)
        self.trend_linear = nn.Linear(lookback, output_dim)
        self.seasonal_linear = nn.Linear(lookback, output_dim)
    ...
    def forward(self, x, adj=None):
        ...
        # Moving average for trend extraction (kernel size = 25 for 5-min data ~ 2 hours)
        kernel_size = min(25, T)
        trend = x.unfold(1, kernel_size, 1).mean(dim=-1)  # (B, T-k+1, N)
        # Pad trend back to length T
        pad_size = T - trend.shape[1]
        if pad_size > 0:
            trend = torch.cat([trend[:, :1, :].expand(-1, pad_size, -1), trend], dim=1)
        seasonal = x - trend

        # Reshape: (B*N, T) for linear
        trend_flat = trend.permute(0, 2, 1).reshape(B * N, T)
        seasonal_flat = seasonal.permute(0, 2, 1).reshape(B * N, T)

        # Linear projection
        trend_out = self.trend_linear(trend_flat)  # (B*N, horizon)
        seasonal_out = self.seasonal_linear(seasonal_flat)

        out = trend_out + seasonal_out  # (B*N, horizon)
        return out.reshape(B, N, -1).permute(0, 2, 1)
