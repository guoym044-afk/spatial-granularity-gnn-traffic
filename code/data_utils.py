...
# === New functions for sensor-subset and ablation experiments ===
# Append to data_utils.py

def select_sensor_subset(X, Y, adj, n_sensors, seed=42):
    """Randomly select n_sensors from the original graph (no averaging).
    Returns X_sub, Y_sub, adj_norm, selected_indices."""
    rng = np.random.RandomState(seed)
    n_total = X.shape[2]
    selected = np.sort(rng.choice(n_total, n_sensors, replace=False))
    X_sub = X[:, :, selected].copy()
    Y_sub = Y[:, :, selected].copy()
    adj_sub = adj[np.ix_(selected, selected)].copy()
    # Symmetric normalize
    deg = np.sum(adj_sub, axis=1)
    deg_inv_sqrt = np.diag(np.where(deg > 0, deg ** -0.5, 0)).astype(np.float32)
    adj_norm = deg_inv_sqrt @ adj_sub @ deg_inv_sqrt
    return X_sub, Y_sub, adj_norm, selected


def build_feature_only_data(X, Y, adj, labels):
    """Mean-pool features within clusters, keep original graph (207 nodes).
    All 207 sensors get their cluster's mean feature. Y unchanged.
    Returns X_pooled, Y (unchanged), adj (unchanged)."""
    unique_labels = np.unique(labels)
    X_pooled = X.copy()
    for lbl in unique_labels:
        mask = (labels == lbl)
        cluster_mean = X[:, :, mask].mean(axis=2, keepdims=True)  # (T, lookback, 1)
        X_pooled[:, :, mask] = cluster_mean
    # adj unchanged — full 207-node graph
    adj_safe = adj + np.eye(adj.shape[0], dtype=np.float32)
    deg = np.sum(adj_safe, axis=1)
    deg_inv_sqrt = np.diag(np.where(deg > 0, deg ** -0.5, 0)).astype(np.float32)
    adj_norm = deg_inv_sqrt @ adj_safe @ deg_inv_sqrt
    return X_pooled, Y, adj_norm


def select_cluster_centroids(X, adj, labels):
    """For each cluster, find the sensor closest to the cluster centroid (by feature mean).
    Returns selected_indices (one per cluster)."""
    unique_labels = np.unique(labels)
    selected = []
    for lbl in unique_labels:
        ...
        if len(mask) == 1:
            ...
            # Centroid: mean feature across all member sensors over all timesteps
            cluster_feats = X[:, :, mask]  # (T, lookback, n_members)
            centroid = cluster_feats.mean(axis=(0, 1))  # (n_members,) — mean over time and lookback
            # Actually we want per-sensor mean over time
            per_sensor_mean = cluster_feats.mean(axis=(0, 1))  # (n_members,)
            centroid_val = per_sensor_mean.mean()
            # Find sensor closest to centroid by absolute difference
            distances = np.abs(per_sensor_mean - centroid_val)
            selected.append(mask[np.argmin(distances)])
    return np.sort(np.array(selected))


def build_graph_only_data(X, Y, adj, labels):
    """Select cluster centroids, use original features (no averaging).
    Target Y is mean-pooled (so evaluation is comparable to coarsened).
    Returns X_centroid, Y_centroid, adj_norm, selected_indices."""
    selected = select_cluster_centroids(X, adj, labels)
    n_clusters = len(selected)
    # X: original features at centroid sensors
    X_centroid = X[:, :, selected].copy()
    # Y: mean-pooled across cluster members (comparable to coarsened evaluation)
    unique_labels = np.unique(labels)
    label_to_idx = {l: i for i, l in enumerate(unique_labels)}
    n_samples, horizon, _ = Y.shape
    Y_centroid = np.zeros((n_samples, horizon, n_clusters), dtype=np.float32)
    for old_n in range(Y.shape[2]):
        new_n = label_to_idx[labels[old_n]]
        Y_centroid[:, :, new_n] += Y[:, :, old_n]
    for new_n in range(n_clusters):
        cluster_size = np.sum(labels == unique_labels[new_n])
        Y_centroid[:, :, new_n] /= max(cluster_size, 1)
    # Adj: coarsened between centroid sensors
    adj_coarse = adj[np.ix_(selected, selected)].copy()
    # Also add edges between centroids if any member was connected
    for i, lbl_i in enumerate(unique_labels):
        members_i = np.where(labels == lbl_i)[0]
        for j, lbl_j in enumerate(unique_labels):
            if i == j:
                continue
            members_j = np.where(labels == lbl_j)[0]
            # Check if any edge exists between members
            sub_adj = adj[np.ix_(members_i, members_j)]
            if np.any(sub_adj > 0):
                adj_coarse[i, j] = 1.0
    deg = np.sum(adj_coarse, axis=1)
    deg_inv_sqrt = np.diag(np.where(deg > 0, deg ** -0.5, 0)).astype(np.float32)
    adj_norm = deg_inv_sqrt @ adj_coarse @ deg_inv_sqrt
    return X_centroid, Y_centroid, adj_norm, selected
